...
# 注意获取分类结果的写法 classes[np.squeeze(train_set_y[:, index])].decode("utf-8")
print("y = " + str(train_set_y[:, index]) + ", it's a '" +
      classes[np.squeeze(train_set_y[:, index])].decode("utf-8") + "' picture.")


#### view matrix/vector dimensions ###
...
def model(X_train, Y_train, X_test, Y_test, num_iterations=2000, learning_rate=0.5, print_cost=False):
    ...
    parameters, grads, costs = optimize(
        w, b, X_train, Y_train, num_iterations, learning_rate, print_cost=False)
...
print("y = " + str(test_set_y[0, index]) + ", you predicted that it is a \"" + (
    "cat" if str(Y_prediction_test_result) == "1.0" else "non-cat") + "\" picture.")
